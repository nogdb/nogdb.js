...
NAN_METHOD(Edge::create)
{
    ...
    else if (info.Length() == 4  && txnType->HasInstance(info[0]->ToObject()) 
                            && info[1]->IsString() && info[2]->IsObject() && info[3]->IsObject())
    {
        Txn *txn = Nan::ObjectWrap::Unwrap<Txn>(info[0]->ToObject());
        ...
        try {
            nogdb::RecordDescriptor recD = nogdb::Edge::create(*txn->base, className,srcRecordD,dstRecordD);
            info.GetReturnValue().Set(v8RecordDescriptor(recD));
        } catch ( nogdb::Error& err ) {
            Nan::ThrowError(err.what());
        } 
